...
class AMRDispatcherService {
  ...
  async dispatchNextTask() {
    try {
      ...
      const optimalAMR = await DispatchStrategy.selectOptimalAMR(idleAMRs, task, this.graph);
  ...
  async canAssignTask(amr, task) {
    try {
      const path = await this.pathfinder.findPath(amr.currentNode, task.pickupNode, amr.amrId);

      if (!path) {
        return { success: false, reason: 'No path available' };
      }

      const isAvailable = await PathReservationService.isPathAvailable(path, amr.amrId);
  ...
  async assignTask(amr, task, path) {
    try {
      const reservation = await PathReservationService.reservePath(amr.amrId, path, task.taskId);
  ...
  start() {
    ...
    this.dispatchTimer = setInterval(() => this.dispatchNextTask(), config.dispatcher.interval);
