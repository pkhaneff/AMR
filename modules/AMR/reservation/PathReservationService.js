...
class PathReservationService {
  async releaseNode(nodeId, amrId) {
    try {
      ...
      if (reservation && reservation.amrId === amrId) {
        await ReservationRepository.releaseNode(nodeId);
        AMRLogger.reservation('Node released', { amrId, nodeId });
        return true;
      }
      return false;
    } catch (error) {
  ...
  async reservePath(amrId, path, taskId) {
    try {
      ...
      const expiresAt = TimeCalculator.addMilliseconds(TimeCalculator.now(), config.reservation.expiryTime);
