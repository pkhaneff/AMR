...
const PathfindingOrchestrator = require('../pathfinding/PathfindingOrchestrator');
...
class AMRService {
  ...
  async generatePathWithConstraints(start, end, action, amrId) {
    this.initialize();
    ...
    const path = await PathfindingOrchestrator.findPathWithConstraints(this.pathfindingService, start, end, amrId);
   ...
   */
  async validateAndPreparePath(start, end, action, amrId = null) {
    try {
      this.initialize();
      ...
      let path;
      if (amrId) {
        path = await PathfindingOrchestrator.findPathWithConstraints(this.pathfindingService, start, end, amrId);
      } else {
        path = this.pathfindingService.findPath(start, end);
      }
