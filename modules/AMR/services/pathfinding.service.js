class PathfindingService {
  ...
  findPath(startId, endId, options = {}) {
    ...
    while (openSet.size > 0) {
      ...
      this.processNeighbors(currentNode, endNode, openSet, closedSet, gScore, fScore, cameFrom, restrictedNodes);
    }

    return null;
  }

  processNeighbors(currentNode, endNode, openSet, closedSet, gScore, fScore, cameFrom, restrictedNodes) {
    currentNode.neighbors.forEach(({ nodeId, distance }) => {
      if (closedSet.has(nodeId) || restrictedNodes.includes(nodeId)) {
        return;
      }
